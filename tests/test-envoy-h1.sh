#!/bin/sh
# Ingress --HTTP/1.1--> Envoy --HTTP/1.1--> java-demo
# While below command is running, restart the java-demo deployment:
#   You should see 503 errors in oha.
...
oha -c 100 -z 10m -m POST -d '{}' -H "Content-Type: application/json" -H "Host: envoy" "http://$LOAD_BALANCER_IP/java-demo/sleep?min=50&max=200"
